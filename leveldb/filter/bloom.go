...
func (f bloomFilter) Contains(filter, key []byte) bool {
	...
	// 使用编码后的k，以至于能够读取使用不同参数创建的bloom filter生成的filter
	k := filter[nBytes]
	if k > 30 {
		// k=30 表示
...
func (f bloomFilter) NewGenerator() FilterGenerator {
	...
	// 如果m和n确定的话，k的最优解是 (m/n)ln(2)，k表示哈希函数个数
	// m为二进制的位数；n为集合中的元素数量
...
func (g *bloomFilterGenerator) Generate(b Buffer) {
	// Compute bloom filter size (in both bits and bytes), bloom filter size: m
	// len(g.keyHashes)是插入的数据量n；nBits是bloom filter数组的容量m
	...
	// 最后一位用来保存k
	dest[nBytes] = g.k
	// 将key的hash值映射到bloom filter的二进制数组中
